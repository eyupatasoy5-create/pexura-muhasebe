...
function normalizeTip(tip){
  const value=String(tip||"").toLowerCase();
  return ["satis","iade","alis","alis_iade"].includes(value) ? value : "satis";
}
...
function calcFaturaTotals(){
  const pb=fPara.value;
  const araToplam=FATURA_SATIRLAR.reduce((t,s)=>t+toNum(s.satir_tutar),0);
  const kdvToplam=FATURA_SATIRLAR.reduce((t,s)=>t+(toNum(s.satir_tutar)*Math.min(100,Math.max(0,toNum(s.kdv_oran)))/100),0);
  const iskonto=Math.min(Math.max(0,toNum(document.getElementById('fIskonto')?.value||0)),araToplam+kdvToplam);
  const genelToplam=Math.max(0,araToplam+kdvToplam-iskonto);
  const karTop=FATURA_SATIRLAR.reduce((t,s)=>t+(toNum(s.birim_fiyat)-toNum(s.alis_snapshot))*toNum(s.miktar),0)-iskonto;
  const setMoney=(id,value)=>{const el=document.getElementById(id);if(el)el.textContent=fmt(value,pb);};
  setMoney('fAraToplam',araToplam);
  setMoney('fKdvToplam',kdvToplam);
  setMoney('fIskontoToplam',iskonto);
  setMoney('fGenel',genelToplam);
  setMoney('fKar',karTop);
  const karVal=document.getElementById('fKarVal');
  if(karVal)karVal.textContent=fmt(karTop,pb);
  return genelToplam;
}
...
function resetFaturaForm() {
  ...
  const vade=document.getElementById('fVade'); if(vade)vade.value='';
  const isk=document.getElementById('fIskonto'); if(isk)isk.value='0';
  const kur=document.getElementById('fKur'); if(kur)kur.value='1';
...
function renderFaturalar(){
  ...
  list.forEach(f=>{
    ...
    const tipLabels={satis:'Satış',iade:'Satış İade',alis:'Alış',alis_iade:'Alış İade'};
    const tipText = tipLabels[normalizeTip(f.tip)] || f.tip;
    const paymentText = f.iptal_edildi ? 'İPTAL' : (f.odeme_durumu || 'Ödenmedi');

    tr.innerHTML=`
      <td data-label="Tarih">${formatTRDateTime(f.tarih)}</td>
      <td data-label="Müşteri">${cariAd}</td>
      <td data-label="Tip"><span class="tag">${tipText}</span><br><small>${paymentText}</small></td>
      ...
      <td data-label="İşlem">
        <div class="btn-group">
          ...
          <button class="danger" data-del="${f.id}">İptal</button>
        </div>
      </td>`;
  ...
  faturaListe.querySelectorAll("[data-del]").forEach(btn=>{
    btn.onclick = async () => {
      const id = btn.dataset.del;
      const invoice = FATURALAR.find(x=>x.id==id);
      if(invoice?.iptal_edildi) return showToast("Fatura zaten iptal edilmiş.","info");
      const reason = prompt("Fatura silinmeyecek; ters stok kaydıyla iptal edilecek.\nİptal nedenini yazın:");
      if(reason===null) return;
      if(reason.trim().length<3) return showToast("İptal nedeni en az 3 karakter olmalı.","warning");
      if(!confirm("Fatura iptal edilip stok hareketleri ters çevrilsin mi?")) return;
      const {error}=await supa.rpc("cancel_invoice_transaction",{p_invoice_id:id,p_reason:reason.trim()});
      if(error) return showToast(error.message,"error");
      showToast("Fatura ters kayıtla iptal edildi.","success");
...
window.renderHistory = () => {
  ...
  (STOK_LOGS || []).forEach(s => {
    ...
    allEvents.push({
      ...
      amount: s.miktar_degisim || s.miktar || s.degisim || 0,
      currency: '',
      color: '#f59e0b'
    });
  });
